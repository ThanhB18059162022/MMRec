...
class SMORE(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        image_adj_file = os.path.join(
            dataset_path, "image_adj_{}_{}.pt".format(self.image_knn_k, self.sparse)
        )
        text_adj_file = os.path.join(
            dataset_path, "text_adj_{}_{}.pt".format(self.text_knn_k, self.sparse)
        )
        ...
        if self.v_feat is not None:
            ...
            if os.path.exists(image_adj_file):
                image_adj = torch.load(image_adj_file, weights_only=True)
            else:
                image_adj = build_sim(self.image_embedding.weight.detach())
                image_adj = build_knn_normalized_graph(
                    image_adj,
                    topk=self.image_knn_k,
        ...
        if self.t_feat is not None:
            ...
            if os.path.exists(text_adj_file):
                text_adj = torch.load(text_adj_file, weights_only=True)
            else:
                text_adj = build_sim(self.text_embedding.weight.detach())
                text_adj = build_knn_normalized_graph(
                    text_adj,
                    topk=self.text_knn_k,
                    is_sparse=self.sparse,
                    norm_type="sym",
                )
        ...
        self.image_complex_weight = nn.Parameter(
            torch.randn(1, self.embedding_dim // 2 + 1, 2, dtype=torch.float32)
        )
        self.text_complex_weight = nn.Parameter(
            torch.randn(1, self.embedding_dim // 2 + 1, 2, dtype=torch.float32)
        )
    ...
    def max_pool_fusion(self):
        ...
        combined_values, _ = torch.max(
            torch.stack((combined_values_image, combined_values_text)), dim=0
        )
    ...
    def spectrum_convolution(self, image_embeds, text_embeds):
        ...
        #   Uni-modal Denoising
        image_conv = torch.fft.irfft(
            image_fft * image_complex_weight,
            n=image_embeds.shape[1],
            dim=1,
            norm="ortho",
        )
        text_conv = torch.fft.irfft(
            text_fft * text_complex_weight, n=text_embeds.shape[1], dim=1, norm="ortho"
        )

        #   Cross-modality fusion
        fusion_conv = torch.fft.irfft(
            text_fft * image_fft * fusion_complex_weight,
            n=text_embeds.shape[1],
            dim=1,
            norm="ortho",
        )
    ...
    def forward(self, adj, train=False):
        ...
        image_conv, text_conv, fusion_conv = self.spectrum_convolution(
            image_feats, text_feats
        )
        image_item_embeds = torch.multiply(
            self.item_id_embedding.weight, self.gate_v(image_conv)
        )
        text_item_embeds = torch.multiply(
            self.item_id_embedding.weight, self.gate_t(text_conv)
        )
        fusion_item_embeds = torch.multiply(
            self.item_id_embedding.weight, self.gate_f(fusion_conv)
        )
        ...
        if self.sparse:
            for i in range(self.n_layers):
                fusion_item_embeds = torch.sparse.mm(
                    self.fusion_adj, fusion_item_embeds
                )
        else:
            for i in range(self.n_layers):
        ...
        fusion_att_v, fusion_att_t = self.query_v(fusion_embeds), self.query_t(
            fusion_embeds
        )
        ...
        image_prefer, text_prefer, fusion_prefer = (
            self.dropout(image_prefer),
            self.dropout(text_prefer),
            self.dropout(fusion_prefer),
        )
        ...
        side_embeds = torch.mean(
            torch.stack([agg_image_embeds, agg_text_embeds, fusion_embeds]), dim=0
        )
